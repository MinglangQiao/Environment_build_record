...
## run
```
th run_model.lua -input_image i226.jpg

## python2 查看结果, 在网页上还可以调节参数
...
th run_model.lua -input_dir ~/1_caption_saliency_ref_code/densecap-master/vis/data/ -output_dir ~/1_caption_saliency_ref_code/densecap-master/vis/data/


```

## env build
0 删除原来的lua,用lua命令查看lua版本，用whereis lua查看lua的路径，然后去/usr/bin/下手动删除lua和lua5.1的文件，然后安装lua5.3./
去[这里](http://www.lua.org/download.html)下载lua
```
...
1 安装torch, 加上from后深度会快一些 (ok)
```
luarocks install torch --from=http://rocks.moonscript.org/dev
```
...
用这个命令
```
~/torch/install/bin/luarocks install nn --from=http://rocks.moonscript.org/dev
```
结果又出现
```
lnn.c:4:23: fatal error: nanomsg/nn.h: 没有那个文件或目录
```
则需要加上sudo
```
...
3 安装image (ok)
```
~/torch/install/bin/luarocks install image --from=http://rocks.moonscript.org/dev
```

4 安装lua-cjson （参考adaptive attention,已安装, ok）

去[这里](https://www.kyne.com.au/~mark/software/lua-cjson.php)下载lua-cjson-2.1.0.tar.gz， 然后
```
cd lua-cjson-2.1.0
~/torch/install/bin/luarocks make
```

5 安装 https://raw.githubusercontent.com/qassemoquab/stnbhwd/master/stnbhwd-scm-1.rockspec
```
...
```
所以需要对gcc降级, 查看原来gcc的版本得怎么看
```
whereis gcc
gcc: /usr/bin/gcc /usr/lib/gcc /usr/local/cuda-8.0/bin/gcc /usr/share/man/man1/gcc.1.gz

/usr/bin/gcc -v # 发现是6.5， 我直接把这个文件给删除了
...
```
还是不行，参考[这个教程](https://stackoverflow.com/questions/42605658/error-no-results-matching-query-were-found-for-luarocks-in-ubuntu)， 先把[github仓库](https://github.com/qassemoquab/stnbhwd)下载下来, 然后进入
```
~/torch/install/bin/luarocks install ./stnbhwd-scm-1.rockspec
```
网络不好的时候多试几次，最后终于可以的


6 安装 https://raw.githubusercontent.com/jcjohnson/torch-rnn/master/torch-rnn-scm-1.rockspec (ok)
```
