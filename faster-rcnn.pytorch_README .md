...
python demo.py --net vgg16 \
               ...
               --cuda --load_dir ./data/PASCAL_VGG_16_1_1/ --net vgg16
...
git clone -b pytorch-1.0 git@github.com:jwyang/faster-rcnn.pytorch.git

conda create -n yang_faster python=3.6

source activate yang_faster
...
```
then ref [faster-rcnn.pytorch 小白踩坑](https://blog.csdn.net/u010826850/article/details/103666308) 和 [记pytorch版faster rcnn配置运行中的一些坑](https://www.cnblogs.com/FZfangzheng/p/10852141.html)
```
cd data

git clone https://github.com/cocodataset/cocoapi # 不能用浏览器下载

cd cocoapi/PythonAPI
make
make install

然后把pycocotools和pycocotools.egg-info文件夹复制到./lib文件下，替换调原来的pycocotools和pycocotools.egg-info文件夹
```
...
if 
```
File "/home/ml/1_detection_method/faster-rcnn.pytorch/lib/model/roi_layers/nms.py", line 3, in <module>                                          
    from model import _C                                                                                                                           
ImportError: /home/ml/1_detection_method/faster-rcnn.pytorch/lib/model/_C.cpython-36m-x86_64-linux-gnu.so: undefined symbol: _ZN2at19UndefinedTenso
rImpl10_singletonE
```
then, cd ./lib
```
...
```
参考[issue503](https://github.com/jwyang/faster-rcnn.pytorch/issues/503
), 要把pytorch降低到1.0.0, 然后重新安装

```
conda install pytorch==1.0.0 -c pytorch

rm -rf lib/build/
python lib/setup.py build develop
```
bingo !
...
```
其中在./lib/model/faster_rcnn里面有 def create_architecture(self): 在faster_rcnn.py里面；

而vgg16.py里面的class vgg16() 继承了faster_rcnn.py里class _fasterRCNN()这个类
